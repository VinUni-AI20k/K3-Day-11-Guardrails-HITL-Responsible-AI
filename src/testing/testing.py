...
async def run_comparison():
    ...
    # --- Protected agent ---
    # Same prompts, same model, only the guardrail plugins differ — that is
    # what makes the before/after table evidence rather than anecdote.
    print("=" * 60)
    ...
    print(
        f"Input guardrail blocked {input_plugin.blocked_count}/"
        f"{input_plugin.total_count}; output guardrail redacted "
        f"{output_plugin.redacted_count}/{output_plugin.total_count}"
    )
...
class SecurityTestPipeline:
    ...
    async def run_all(self, attacks: list = None) -> list:
        ...
        # Sequential on purpose: parallel fire would trip the rate limiter
        # and the results would measure the limiter, not the guardrails.
        results = []
