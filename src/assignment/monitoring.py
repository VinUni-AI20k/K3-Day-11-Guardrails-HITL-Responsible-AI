"""
Assignment 11 — Monitoring and alerts implementation.
...
class MonitoringAlert:
    ...
    def check_metrics(self) -> list[Alert]:
        """Compute rates and append alerts when thresholds are exceeded."""
        ...
        if block_rate >= self.block_rate_threshold:
            self.alerts.append(Alert("block_rate", block_rate, self.block_rate_threshold, "Blocked request rate exceeded threshold"))
        if self.rate_limit_hits >= self.rate_limit_hit_threshold:
            self.alerts.append(Alert("rate_limit_hits", float(self.rate_limit_hits), float(self.rate_limit_hit_threshold), "Rate-limit hits exceeded threshold"))
        if judge_rate >= self.judge_fail_rate_threshold and self.judge_checks:
            self.alerts.append(Alert("judge_fail_rate", judge_rate, self.judge_fail_rate_threshold, "LLM judge failure rate exceeded threshold"))
        return self.alerts
